...
The dashboard at `gc dashboard serve` would normally show all of this in a tidier UI. On my v1.0.0 run the dashboard was entirely unresponsive ([GH#1168](https://github.com/gastownhall/gascity/issues/1168), fixed). YMMV: try `gc dashboard serve` first; if the panels populate cleanly, use it. If not, the overview script is the fallback used throughout the rest of the tutorial.
