...
That is not a failure. The controller runs an fsnotify watcher ([GH#926](https://github.com/gastownhall/gascity/pull/926), merged) that picks up edits to `pack.toml` and prompt templates within milliseconds. By the time you run `gc reload`, the in-memory config already matches disk.
